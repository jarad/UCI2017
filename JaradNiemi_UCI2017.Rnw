...
\title[Fully Bayes RNAseq analysis]{Fully Bayesian analysis for\\hierarchical count regression models}
...
{\small
Dr. Dan Nettleton, Dr. Will Landau, Eric Mittman, and Dr. Ignacio Alvarez-Castro
}
...
Experimental design:  4 varieties, 2 flow cells, 2 replicates per variety per flow cell
...
\begin{itemize}[<+->]
\item $\beta_{g1}$ is the parental mean
\item $\beta_{g2}$ is the half difference of hybrid mean vs M
\item $\beta_{g3}$ is the half difference of hybrid mean vs B
\item $\beta_{g4}$ is the half difference between hybrids
\item $\beta_{g5}$ is the flow cell block effect 
\end{itemize}
\end{frame}
...
There are $2(L+1)$ hyperparameters and we assign the following independent priors
\[ \begin{array}{rll}
...
\item Calculate sufficient ``statistics'' \pause $\to$ reduction \pause - possible speedup is $[G-1]/\log_2(G)\pause \approx 2,500$
\end{itemize}
...
\begin{itemize}
\item \url{https://github.com/jarad/UCI2017}
